...
def _lottery_list(entries: list[tuple[dict, datetime, datetime]], now: datetime, view: str = "active") -> str:
    ...
    for item, start, deadline in active:
        ...
        cards.append(
            ...
            f'<div class="tags">{tag_html}</div><a class="apply-link" href="{url}" target="_blank" rel="noreferrer">\U0001f310 Web\u3067\u5fdc\u52df\u3059\u308b</a><button class="hide-lottery" type="button" data-lottery-id="{item_id}" onclick="try{{const k=\'card-lottery-hidden\';const s=new Set(JSON.parse(localStorage.getItem(k)||\'[]\'));s.add(this.dataset.lotteryId);localStorage.setItem(k,JSON.stringify([...s]));}}catch(e){{}}this.closest(\'.lottery-card\').style.display=\'none\';">\U0001f648 \u3053\u306e\u62bd\u9078\u3092\u975e\u8868\u793a</button></article>'
        )
    filters = '<button id="reset-hidden" class="reset-hidden" type="button" onclick="try{localStorage.removeItem(\'card-lottery-hidden\');}catch(e){}location.reload();">\U0001f504 \u975e\u8868\u793a\u3092\u3059\u3079\u3066\u623b\u3059</button>' if is_active else ''
