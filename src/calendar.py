...
def _lottery_list(entries: list[tuple[dict, datetime, datetime]], now: datetime, view: str = "active") -> str:
    ...
    for item, start, deadline in active:
        ...
        hide_id = f"hide-{item_id}"
        cards.append(
            f'<input id="{hide_id}" class="hide-toggle" type="checkbox" data-lottery-id="{item_id}"><article id="lottery-{item_id}" class="lottery-card" style="{_application_status_style(view)}" data-status="{view}" data-condition="{condition_kind}" data-application="{application}">'
            ...
            f'<div class="tags">{tag_html}</div><a class="apply-link" href="{url}" target="_blank" rel="noreferrer">\U0001f310 Web\u3067\u5fdc\u52df\u3059\u308b</a><label class="hide-control" for="{hide_id}">\U0001f648 \u3053\u306e\u62bd\u9078\u3092\u975e\u8868\u793a</label></article>'
        )
    filters = '<a class="reset-hidden-v2" href="./">\U0001f504 非表示をすべて戻す</a>' if is_active else ''
    ...
    # The visible hide control itself is CSS-only. It works even if JavaScript fails.
    card_styles += '<style>.hide-toggle{position:absolute;opacity:0;pointer-events:none}.hide-toggle:checked + .lottery-card{display:none!important}.hide-control{display:inline-block;padding:8px 2px 0;color:#4b5563;font-size:.78rem;font-weight:700;cursor:pointer}.reset-hidden-v2{display:inline-block;padding:7px 4px;color:#4b5563;font-size:.78rem;font-weight:700;text-decoration:underline}</style>'
...
def build_calendar(items: list[dict], destination: Path, timezone: str = "Asia/Tokyo"):
    ...
    page = f'''<!doctype html>
<html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>\u30ab\u30fc\u30c9\u62bd\u9078\u30ab\u30ec\u30f3\u30c0\u30fc</title><script defer src="./app.js"></script><style>
...
'''
    (destination / "app.js").write_text(app_script, encoding="utf-8")
