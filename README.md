...
| `RESULT_DISCORD_WEBHOOK_URL` | 抽選結果の確認通知を別チャンネルへ送りたい場合のみ設定。未設定なら本番Webhookへ送信 |
...
幹部用の `admin.html` で各抽選を開き、**「その他（抽選結果の通知）」** に次を登録できます。

- 結果発表日時
- 結果ページURL（空欄なら応募ページを開く）
- Discord通知を送る、のチェック

チェックを入れた抽選だけ、結果発表日時を過ぎた最初の毎朝9時の本番更新で1回だけ通知します。通知済みの記録は保存されるため、翌日以降に重複して届きません。
